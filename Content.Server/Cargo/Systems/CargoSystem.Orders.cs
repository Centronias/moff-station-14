...
namespace Content.Server.Cargo.Systems
{
    public sealed partial class CargoSystem
    {
        ...
        private void OnInteractUsing(EntityUid uid, CargoOrderConsoleComponent component, ref InteractUsingEvent args)
        {
            ...
            if (GetCargoHost(uid) is not var (station, bank, _))
            {
                // No linked server to add money to.
                ConsolePopup(args.User, Loc.GetString("cargo-server-error-not-linked"));
            ...
            UpdateBankAccount((station, bank), (int) price, component.Account); // Moffstation - Cargo Server
            QueueDel(args.Used);
            args.Handled = true;
        }
        ...
        private void OnApproveOrderMessage(EntityUid uid, CargoOrderConsoleComponent component, CargoConsoleApproveOrderMessage args)
        {
            ...
            {
                ConsolePopup(args.Actor, Loc.GetString("cargo-server-error-not-linked"));
                // Moffstation - End
                PlayDenySound(uid, component);
                return;
            }
            ...
            var ev = new CargoApprovedOrderMessage(
                ...
                shouldAnnounceFulfillment: !_emag.CheckFlag(uid, EmagType.Interaction),
                    uid,
                    player,
                    _protoMan.TryIndex(component.Account, out var account) ? account.RadioChannel : CargoOrderConsoleComponent.BaseAnnouncementChannel
            );
        ...
        {
            if (GetCargoHost(uid) is not {} server) // Moffstation - Cargo Server
                return;
        ...
        {
            if (GetCargoHost(entity.Owner) is not { } server ||
